...
│                                                              │
│  seenIds: Set<string>  ─── 所有已处理过的 tool_use_id        │
│  replacements: Map<string, string> ─── 被替换的结果           │
│                        及其精确替代内容                        │
│                                                              │
│  三种状态：                                                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │  mustReapply  │  │   frozen     │  │    fresh     │       │
│  │ (在 seenIds   │  │ (在 seenIds  │  │ (不在 seenIds│       │
│  │  且有替换内容) │  │  但无替换)   │  │  中的新结果)  │       │
│  │              │  │              │  │              │       │
│  │ → 重新应用    │  │ → 不可变更   │  │ → 可被选中   │       │
│  │   缓存的替换  │  │   原样保留   │  │   进行替换   │       │
...
│ 第1层：单工具结果持久化                                           │
│                                                                  │
│  工具执行 → 结果 > 阈值？→ 持久化到磁盘 + 2KB 预览                │
│  阈值 = min(工具声明值, 50K) 或 GrowthBook 覆盖                   │
│  特例：Read (Infinity), 图片 (跳过)                               │
├─────────────────────────────────────────────────────────────────┤
│ 第2层：单消息聚合预算                                              │
│                                                                  │
│  API 调用前 → 每个"API 消息"的 tool_result 总量 > 200K？          │
│  → 按大小降序持久化 fresh 结果，直到总量 ≤ 200K                    │
│  → 状态冻结：seen 结果的命运永不改变（prompt cache 稳定性）         │
├─────────────────────────────────────────────────────────────────┤
│ 第3层：上下文窗口追踪                                              │
│                                                                  │
│  tokenCountWithEstimation() = 精确 usage + 增量粗略估算            │
│  → 驱动自动压缩（ch09）、微压缩（ch11）等决策                      │
│  → 并行工具调用：同 ID 回溯修正，避免系统性低估                    │
├─────────────────────────────────────────────────────────────────┤
│ 第4层：自动压缩 / 微压缩（详见第9-11章）                           │
│                                                                  │
│  上下文接近窗口限制 → 压缩历史消息 / 清理旧工具结果                 │
└─────────────────────────────────────────────────────────────────┘
```
