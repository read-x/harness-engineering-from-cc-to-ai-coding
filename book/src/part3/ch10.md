...
```
文件在压缩前被读取过？
├── 否 → 【不恢复】文件不在 readFileState 中
└── 是
    ├── 是 plan 文件？
    │   └── 是 → 【排除】通过 Plan 附件独立恢复（见 10.4）
    ├── 是 CLAUDE.md 记忆文件？
    │   └── 是 → 【排除】通过系统提示词注入
    ├── 已在保留的消息尾部？
    │   └── 是 → 【排除】模型已能看到，无需重复
...
```
步骤 1：快照并清空
  ├── cacheToObject(readFileState) → 保存文件状态快照
  ├── readFileState.clear()       → 清空文件缓存
  └── loadedNestedMemoryPaths.clear() → 清空记忆路径

步骤 2：并行生成附件
  ├── [并行] createPostCompactFileAttachments → 文件恢复附件
  └── [并行] createAsyncAgentAttachmentsIfNeeded → 异步 agent 附件

步骤 3：串行生成附件
  ├── createPlanAttachmentIfNeeded    → 计划内容附件
  ├── createPlanModeAttachmentIfNeeded → 计划模式附件
  └── createSkillAttachmentIfNeeded   → 已调用技能附件

步骤 4：Delta 完整重播
  ├── getDeferredToolsDeltaAttachment([], 'compact_full') → 延迟工具
  ├── getAgentListingDeltaAttachment([])                  → Agent 列表
  └── getMcpInstructionsDeltaAttachment([])               → MCP 指令

步骤 5：所有附件合并为 postCompactFileAttachments
  └── 随压缩后的第一条消息发送给模型
```
