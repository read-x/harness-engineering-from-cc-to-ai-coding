...
maybeTimeBasedMicrocompact(messages, querySource)
  │
  ├─ evaluateTimeBasedTrigger() → null? → 返回 null（不触发）
  │
  ├─ collectCompactableToolIds(messages) → 收集所有可压缩工具 ID
  │
  ├─ keepRecent = Math.max(1, config.keepRecent)  ← 至少保留 1 个
  │   └─ 原因: slice(-0) 返回整个数组（JavaScript 怪癖），
  │      而清除所有结果会让模型丧失工作上下文
  │
  ├─ keepSet = compactableIds.slice(-keepRecent)  ← 保留最近 N 个
  ├─ clearSet = 其余全部清除
  │
  ├─ 遍历 messages，将 clearSet 中的 tool_result.content
  │   替换为 '[Old tool result content cleared]'
  │
  ├─ suppressCompactWarning()  ← 抑制上下文压力警告
  ├─ resetMicrocompactState() ← 重置缓存 MC 状态
  └─ notifyCacheDeletion()    ← 通知缓存中断检测器
```
...
```
┌──────────┐         ┌────────────┐        ┌──────────────┐
│ microcom- │         │  claude.ts  │        │  Anthropic   │
│ pact.ts   │         │ (API layer) │        │  API Server  │
└─────┬─────┘         └──────┬──────┘        └──────┬───────┘
      │                      │                      │
      │ ① 注册 tool_results  │                      │
      │ (registerToolResult) │                      │
