...
> Agent 系统最脆弱的部分不是模型推理，而是与模型的通信。网络不稳定、API 过载、流式中断、认证过期——任何一个环节的失败都会中断整个 Agent Loop。Claude Code 在 `withRetry.ts`（822 行）和 `claude.ts`（2800+ 行）中构建了一套生产级的通信韧性系统，本章将完整解剖这套系统。
