...
> Teams 系统的实现细节——三种后端如何选择、Mailbox 文件通信协议、Leader 权限代理审批、团队共享记忆——详见 20.9 节。
...
## 20.9 Teams 实现细节：后端、通信、权限与记忆

> 本节是 20.3（队友 Agent / Agent Swarms 概述）的实现层深入。20.3 回答"Teams 是什么"——平面结构团队、TeamCreateTool、TeammateAgentContext 类型；本节回答"Teams 怎么跑起来"——进程管理、通信协议、权限同步、共享记忆的具体工程实现。
>
