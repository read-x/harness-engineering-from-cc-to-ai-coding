...
└──────┬──────────────────────────────────────────────────┘
       ...
       ▼
┌──────────────────────┐
│ 批次 1 (并发安全)     │  ← Read A, Read B, Grep C
│ isConcurrencySafe: ✓ │    三个只读工具合并为一批
├──────────────────────┤
│ 批次 2 (串行)         │  ← Bash D
│ isConcurrencySafe: ✗ │    写入工具独占一批
├──────────────────────┤
│ 批次 3 (并发安全)     │  ← Read E
│ isConcurrencySafe: ✓ │    新的只读批次
├──────────────────────┤
│ 批次 4 (串行)         │  ← Edit F
│ isConcurrencySafe: ✗ │    写入工具独占一批
└──────────────────────┘
...
**图 4-1：partitionToolCalls 分区逻辑图。** 连续的并发安全工具被合并到同一批次，非并发安全工具各自独占一批。
